 ...
 **/
//call sample: set_required_nb_seen (g, N, 0, 0);
...
/**
*Appelee ssi on vient de parcourir un antimiroir.
*	Change la direction en fonction de celle initialie
*
**/
...
/**
*Appelee ssi on vient de parcourir un miroir.
*	Change la direction en fonction de celle initialie
*
**/
...
bool add_monster(game g, content monster, int x, int y){
	...
	if(g->matrice[x][y] == MIRROR || g->matrice[x][y] == ANTIMIRROR){
		return false;
	} else if(x >=COLONNE || x < 0 || y < 0 || y >= LINE){	//Si la position x ou y est en dehors de la matrice
		return false;
	} else {//Si tout est ok
